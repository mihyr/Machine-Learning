...
class GMM():
    ...
    def _e_step(self, features):
        ...
        """
        #det shape of features
        f_shape = np.shape(features)[0]
        f_shape1= np.shape(features)[1]
        count = 1
        num = self.n_clusters
        #init posterior array
        posterior = np.empty((f_shape,num*count))
        posterior_shape = np.shape(posterior)[1]

        #iter for posterior_shape
        for elem in range(0,posterior_shape):
            posterior[:,elem] = self._posterior(features,elem)
        #count+=1
        #print(posterior)
        return posterior
    ...
    def _m_step(self, features, assignments):
        ...
        for elem in range(num):
            mean.append(np.empty(f_shape1))

        for m in range(f_shape1):
            for n in range(num):
                assignment_n = assignments[:, n]
                assignment_shape = np.shape(assignment_n)
                mean_numerator = np.sum(features[:, m]*assignment_n, axis=None)
                sum_assignment_n = np.sum(assignment_n, axis=None)
                mean[n][m] = mean_numerator/sum_assignment_n
        
        #covariance
        if self.covariance_type == 'diagonal':
            covariance = np.empty((num, f_shape1))
            for i in range(num):
                f_mean_sub = features - mean[i]
                f_assignment = f_mean_sub.T*assignments[:, i]
                f_dot = np.dot(f_assignment, f_mean_sub)
                #print(f_dot)
                assignment_sum = np.sum(assignments[:, i], axis=None)
                covariance_eval =  f_dot*count/assignment_sum
                covariance[i] = np.diag(covariance_eval,k=0)

        elif self.covariance_type == 'spherical':
            covariance = np.empty((num))
            for i in range(num):
                f_mean_sub = features - mean[i]
                f_assignment = f_mean_sub.T*assignments[:, i]
                f_dot = np.dot(f_assignment, f_mean_sub)
                ...
                assignment_sum = np.sum(assignments[:, i], axis=None)
                covariance_eval =  f_dot*count/assignment_sum
                covariance_diag = np.diag(covariance_eval,k=0)
                covariance[i] = np.mean(covariance_diag)
        else:
            print('other cov type')
            
        #mix_weights
        mix_weights = np.empty(num)
        for elem in range(num):
            w_sum = np.sum(assignments[:,elem], axis=None)
            mix_weights[elem] = w_sum*count/f_shape

        #print(mean, covariance, mix_weights)
        return mean, covariance, mix_weights
    ...
    def _log_likelihood(self, features, k_idx):
        ...
        """
        mv_normal_dist = multivariate_normal.logpdf(features, mean = self.means[k_idx], cov = self.covariances[k_idx])
        m_weight_kid = self.mixing_weights[k_idx]
        m_weight_log = np.log(m_weight_kid)
        #print(mv_normal_dist)
        #print(m_weight_kid)
        #print( mv_normal_dist + m_weight_log)
        return mv_normal_dist + m_weight_log
