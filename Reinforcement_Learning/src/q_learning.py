...
class QLearning:
    ...
    def fit(self, env, steps=1000):
        ...
        Q = np.zeros((available_space, available_actions))
        N = np.zeros((available_space, available_actions))
        rewards = np.zeros((100))
        reward_list = np.array([])
        counter = 0

        env.reset()
        initialize = np.random.choice(available_space)
        trigger = 0
        ...
        while trigger<steps:
          ...
          #for j in range(100):
          if np.random.random() < self._get_epsilon(trigger/steps):
              action = env.action_space.sample()
          elif not np.any(Q[initialize,:]):
            action = env.action_space.sample()
          else:
            action = np.argmax(Q[initialize,:])
          ...
          N[initialize, action] += 1

          disc_sta_prod =  self.discount * np.amax(Q[obs, :]) 
          cal_op =reward+ disc_sta_prod - Q[initialize, action]

          Q[initialize, action]+= 1/N[initialize, action] * cal_op
