...
class KNearestNeighbor():    
    ...
    def predict(self, features, ignore_first=False):
        ...
        """
        row_F = features.shape[0]
        col_T = self.ip_targets.shape[1]

        #init output_label
        output_label = np.empty(shape = (row_F, col_T))

        if self.distance_measure == 'euclidean':
            #calculate euclidean dist
            dist = euclidean_distances(self.ip_features, features)
        elif self.distance_measure == 'manhattan':
            #calculate manhattan dist
            dist = manhattan_distances(self.ip_features, features)
        
        #nested iteration loop
        for f in range(row_F):
            for t in range(col_T):
                
                #check ignore_first condition
                if ignore_first == True:
                    n_range = range(1, self.n_neighbors+1)
                else:
                    n_range = range(self.n_neighbors)

                #create sorted array till n_range
                element_dist = np.sort(dist[:,f])[n_range]
                
                #find elements meeting condition and stack horizontally
                element_find = [np.where(dist[:, f] == d)[0] for d in element_dist]
                #print(element_find)
                element_stack = np.hstack(element_find)
                

                #find unique elements in array
                n_index = np.unique(element_stack)

                #iter target n_output
                n_output = [self.ip_targets[q, t] for q in n_index]
                #print(n_output)

                #check for mean/median/mode
                if self.aggregator == 'mean':
                    mean_op = sum(n_output)/n_index.size
                    #print(mean_op)
                    output_label[f,t]= mean_op

                elif self.aggregator == 'median':
                    median_op = median(n_output)
                    #print(median_op)
                    output_label[f,t]= median_op

                elif self.aggregator == 'mode':
                    mode_op = mode(n_output)
                    #print(mode_op)
                    output_label[f,t]= mode_op
